...
const ChatRoom = () => {
  ...
  const sendMessage = async (e) => {
    ...
    const res = await axios
      .post(`/TT/talk/topic`, JSON.stringify(data), {
    ...
    console.log('ChatRoom >>>>', sendmessge);
        ...
        <CardContent sx={{ width: 600, height: 450 }}>
          {() => {
            const datalist = messageList.map((message) => {
              return auth.token === message.list.no ? (
                <ListItem
                  style={{ width: 400, borderRadius: '10px', backgroundColor: 'greenyellow' }}
                  key={index}
                >
                  <ListItemText>{message.contents}</ListItemText>
                </ListItem>
              ) : (
                <ListItem
                  style={{ width: 400, borderRadius: '10px', backgroundColor: 'skyblue' }}
                  key={index}
                >
                  <ListItemText>{message.contents}</ListItemText>
                </ListItem>
          ...
          {/* //실시간 채팅 말풀선 */}
          <CardContent sx={{ width: 1450, height: 450 }}>
                ...
                <div>
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <Box sx={{ display: 'flex' }}>
                      ...
                      >
                        <Box ml={3} sx={{ fontWeight: 500 }}>
                          메세지ddkljsdfjksdlfjsdflsdjsd;lfkjsdlfkjdsfl/ {sendmessge.contents}
                        </Box>
                        <Box ml={2} mt={6} mr={2} sx={{ fontWeight: 'bold' }}>
                          21/12/25 11:50 // {sendmessge.regTime}
                        </Box>
                      </Box>
                    </Box>
                  </div>
                  ...
                  <div>
                    <Box sx={{ display: 'flex' }} ml={120}>
                      ...
                      >
                        ...
                        <Box ml={3} mr={3} sx={{ fontWeight: 500 }}>
                          메세지ddkljsdfjksdlfjsdflsdjsd;lfkjsdlfkjdsfl
                        </Box>
                      </Box>
                    </Box>
                  </div>
